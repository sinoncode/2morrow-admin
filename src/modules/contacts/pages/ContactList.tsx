            ...
            <Card className="shadow-sm">
                <CardHeader className="flex flex-row flex-wrap items-center gap-3 border-b py-3 px-4">
                    ...
                    <div className="relative w-[240px]">
                        ...
                        <Input placeholder="Search contacts, client, agent…" className="pl-8 h-9 text-sm"
                            value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} />
                    </div>
                    <div className="ml-auto text-xs text-muted-foreground">{filtered.length} contact{filtered.length !== 1 ? "s" : ""}</div>
                </CardHeader>
                        ...
                        <Table className="min-w-[1100px]">
                            <TableHeader>
                                <TableRow className="bg-muted/40 hover:bg-muted/40">
                                    {["Contact ID", "Title & Client", "Type", "Transaction", "Location", "Area", "Price", "Notary", "Agent", "Closing Date", "Stage", "Action"].map((h, i) => (
                            ...
                            <TableBody>
                                {paginated.length === 0 ? (
                                    <TableRow><TableCell colSpan={12} className="py-16 text-center text-muted-foreground text-sm">No contacts match your filters.</TableCell></TableRow>
                                ) : paginated.map((d, i) => (
                                        ...
                                        <TableCell className="text-right pr-4">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-44">
                                                    <DropdownMenuItem className="text-sm gap-2 cursor-pointer"><Eye className="h-3.5 w-3.5" />View Contact</DropdownMenuItem>
                                                    <Link to="/contacts/details">
                                                        <DropdownMenuItem className="text-sm gap-2 cursor-pointer"><Pencil className="h-3.5 w-3.5" />Edit Contact</DropdownMenuItem>
                                                    </Link>
